...
int secho(struct stty *tty, int c)
{
	...
	// lock to prevent data change
	lock();
	// set the char to the buffer
	tty->ebuf[tty->ehead] = c;
	// normal circular data code
	tty->ehead++;
	tty->e_count++;
	tty->ehead %= EBUFLEN;
	// dont forget to unlock
	unlock();
	// reinable transmitting
	enable_tx(tty);
}
...
int do_rx(struct stty *tty)   /* interrupts already disabled */
{
	...
	/****** This code segment uses bput() to echo each input char ************
	  ...
	  bputc(tty->port, '\n');
	 ************************************************************************/
	// lock normal stuff and normal circular buffer stuff
	lock();
	tty->inbuf[tty->inhead]=c;
	tty->inhead++;
	tty->inhead %= INBUFLEN;
	unlock();
	// call secho to save the char we just got
	secho(tty, c);
